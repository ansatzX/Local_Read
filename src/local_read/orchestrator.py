...
def process_and_save(
    ...
    # Normalize once, before writing per-chunk artifacts. Native MinerU JSON
    # stays unchanged and uses its own local page convention.
    if format == "pdf":
        offset = p_start if "p_start" in locals() else 0
        for block in intermediate.get("blocks", {}).values():
            if isinstance(block.get("page"), int):
                block["page"] += offset
        source = intermediate.setdefault("source", {})
        source["path"] = str(Path(file_path).absolute())
        processed_end = p_end if "p_end" in locals() else (source.get("page_count") or 0) - 1
        source["processed_pages"] = list(range(offset + 1, processed_end + 2))
        source["page_count"] = total if "total" in locals() else source.get("page_count")
        intermediate.setdefault("provenance", {}).update({
            "page_numbering": "physical_1_based", "page_scope": "source_file",
            "input_page_offset": offset,
        })

    # Preserve upstream rich outputs and use MinerU's own Markdown reconstruction.
    native_markdown = intermediate.pop("_native_markdown", None)
    native_files = {}
    for key, name in (("_mineru_middle_json", "mineru_middle.json"),
                      ("_mineru_content_list", "mineru_content_list.json")):
        payload = intermediate.pop(key, None)
        if payload is not None:
            path = output_path / name
            path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
            native_files[name.removesuffix(".json")] = str(path)
    ...
    markdown_content = native_markdown if isinstance(native_markdown, str) else markdown_converter.convert()
    ...
    result: dict[str, Any] = {
        ...
        "phys_start": offset if format == "pdf" else getattr(chunk, "phys_start", 0),
        "phys_end": processed_end if format == "pdf" else getattr(chunk, "phys_end", 0),
...
def merge_chunk_markdowns(chunk_results: list[dict[str, Any]]) -> str:
    ...
    for cr in chunk_results:
        ...
        # Simple PDF blocks have page provenance: remove complete overlap
        # pages before rendering, rather than guessing repeated text lines.
        doc = cr.get("intermediate", {})
        if doc.get("provenance", {}).get("page_scope") == "source_file" and not cr.get("native_files"):
            blocks = doc.get("blocks", {})
            order = [key for key in doc.get("reading_order", [])
                     if blocks[key].get("page") not in covered_pages]
            md = MarkdownConverter({**doc, "reading_order": order}, include_metadata=False).convert()
            covered_pages.update(block["page"] for block in blocks.values()
                                 if isinstance(block.get("page"), int))
            original_md = md
...
def merge_chunk_intermediates(source_path: str, format: str, chunk_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for result in chunk_results:
        ...
        if not merged["metadata"]:
            merged["metadata"] = dict(intermediate.get("metadata", {}))
        for kind, annotations in intermediate.get("annotations", {}).items():
            merged.setdefault("annotations", {}).setdefault(kind, []).extend(annotations)
        merged["source"]["page_count"] = max(
            merged["source"]["page_count"], int(result.get("phys_end", offset)) + 1,
            intermediate.get("source", {}).get("page_count") or 0,
        )
        current_pages: set[int] = set()
        processed_pages.update(intermediate.get("source", {}).get("processed_pages", []))
        for old_id in intermediate.get("reading_order", []):
            block = dict(intermediate.get("blocks", {}).get(old_id, {}))
            if isinstance(block.get("page"), int):
                if intermediate.get("provenance", {}).get("page_scope") != "source_file":
                    block["page"] += offset
                if block["page"] in covered_pages:
                    continue
        ...
        covered_pages.update(current_pages)
    merged["source"]["processed_pages"] = sorted(processed_pages)
    merged["provenance"] = {"page_numbering": "physical_1_based", "page_scope": "source_file"}
    return merged
